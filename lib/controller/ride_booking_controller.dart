...
class RideBookingController extends GetxController {
  ...
  bool _shouldUpdateAddress = false;
  ...
  Future<void> applyCoupon() async {
    if (couponTextController.text.isEmpty) {
      Utils.showErrorToast(message: "Please enter a coupon code");
      return;
    }
    ...
    try {
      var response = await ApiProvider().applyCoupon(map);
      Utils.hideLoading();
      if (response.success == true) {
        ...
        Utils.showErrorToast(message: response.message ?? "Invalid Coupon");
      }
    } catch (e) {
      Utils.hideLoading();
      Utils.showErrorToast(message: "Failed to apply coupon");
    }
  }
  ...
  double calculateDiscountedFare(dynamic originalFare) {
    double fare = double.tryParse(originalFare.toString()) ?? 0.0;
    if (isCouponApplied.value) {
      // Formula: Price - (Price * (Percentage / 100))
      double discount = fare * (discountPercentage.value / 100);
  ...
  Future<void> createBooking() async {
    ...
    final Map<String, dynamic> map = {
      ...
      if (isCouponApplied.value) "couponCode": appliedCouponCode.value,
    };
