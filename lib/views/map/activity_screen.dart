  ...
  Widget _buildActionButton(String label, IconData icon,Body item, {bool small = false, bool isWhite = false, bool fullWidth = false, bool isBlack = false}) {
    return GestureDetector(
      onTap: (){
        ...
        Logger().d("dsfdsgdsg");
        if(label == "Rebook" || label == "Rebook Ride"){
          Get.to(
                () =>  RideBookingMainScreen(),
            arguments: {
              'pickup_address': item.pickUpLocation ?? "",
              'pickup_lat': double.tryParse(item.pickUpLatitude.toString()) ?? 0.0,
              'pickup_lng': double.tryParse(item.pickUpLongitude.toString()) ?? 0.0,
              'dest_address': item.destinationLocation ?? "",
              'dest_lat': double.tryParse(item.destinationLatitude.toString()) ?? 0.0,
              'dest_lng': double.tryParse(item.destinationLongitude.toString()) ?? 0.0,
            },
          );
        }
