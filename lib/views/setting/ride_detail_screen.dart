          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Expanded(
                    child: Text("${data.typeOfVechile?.name ?? "Ride"} ride",
                        style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.bold)),
                  ),
                  CircleAvatar(
                    radius: 35,
                    backgroundColor: Colors.grey[200],
                    backgroundImage: data.driver?.profilePicture != null
                        ? NetworkImage(ApiConstants.userImageUrl + data.driver!.profilePicture!)
                        : null,
                    child: data.driver?.profilePicture == null
             ...
             /* Row(
              ...
              if(controller.rideDetail.value.status == 6)...[
                const SizedBox(height: 20),
                Text("Help & safety",
  ...
  Widget _buildInfoRow(IconData icon, String text, {bool showArrow = false}) {
    return Padding(
      ...
      child: GestureDetector(
        onTap: () async{
          if(controller.rideDetail.value.lostItem?.paymentStatus == 1){
            await Get.offNamed(AppRoutes.lostItemMapScreen,arguments: {"requestId":controller.rideDetail.value.lostItem?.id.toString()});
          }else{
            await Get.toNamed(AppRoutes.lostItemScreen,arguments: {"bookingId":controller.bookingId,"driverId": controller.rideDetail.value.driver?.id ?? "","phone": controller.rideDetail.value.driver?.phoneNumber ?? "","item":controller.rideDetail.value.lostItem});
          }
